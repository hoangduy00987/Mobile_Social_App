...
export default function PostListItem({ post, isDetailedPost }: PostListItemProps) {
  ...
  const { mutate: upvote } = useMutation({
    mutationFn: (value: 1 | -1) =>
      createUpvote({ post_id: post.id, comment_id: undefined, vote_type: value }),
    // Optimistic update
  ...
  const {
    ...
    queryKey: ['posts', post.id, 'my-vote'],
    queryFn: () => selectMyVote(post.id, undefined),
    staleTime: 1000 * 60,
    retry: 2,
  ...
  const { mutate: unsave } = useMutation({
    mutationFn: () => deleteSavedPost(post.id),
    // Optimistic update
    onMutate: async (newValue) => {
      // Cancel any outgoing refetches
      await queryClient.cancelQueries({ queryKey: ['posts', post.id, 'saved'] })
  ...
  const {
    ...
    queryKey: ['posts', post.id, 'saved'],
    queryFn: () => selectMySavedPost(post.id),
    staleTime: 1000 * 60,
    retry: 2,
    refetchOnWindowFocus: false,
  })
    ...
    >
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        ...
        <View>
          ...
          {isDetailedPost && (
            <Text style={{ fontSize: 13, color: '#2E5DAA' }}>{`u/${post.author.full_name}`}</Text>
          )}
        </View>
        <Pressable
