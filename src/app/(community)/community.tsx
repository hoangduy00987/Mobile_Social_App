...
const CommunityScreen = () => {
  ...
  const fetchDataCommunity = async () => {
    try {
      const data = await getCommunityById(Number(community_id))
      setInfoCommunity(data)
    } catch (error: any) {
  ...
  const fetchMemberCommunity = async () => {
    try {
      const data = await getMembersById(Number(community_id))
      setInfoMember(data?.members)
    } catch (error: any) {
      console.log('>>> Error: ', error)
    }
  }
  ...
  const fetchMemberPendingCommunity = async () => {
    try {
      const data = await getMembersPendingById(Number(community_id))
      setInfoMemberPending(data?.members)
    } catch (error: any) {
      console.log('>>> Error: ', error)
    }
  }
  ...
  const RemoveMember = async (community_id: number, user_id: number) => {
    try {
      await rejectMember(community_id, user_id)
  ...
  const ApproveMember = async (community_id: number, user_id: number) => {
    try {
      await approveMember(community_id, user_id)
  ...
  const PanelContent = () => (
    <View style={styles.containerPanel}>
      <TouchableOpacity style={styles.btnClose} onPress={() => setOpenPanel(false)}>
      ...
      <View style={styles.containerMember}>
        ...
        {panelType === 'members' &&
          infoMember.map((item, index) => (
            <View
              style={[styles.cardMember, item?.user_id === authUser?.id && styles.cardMemberAd]}
              key={`idx-${index}-${item?.community_id}`}
            >
              <Image
                source={
              ...
              <View style={styles.infoMember}>
                <Text style={{ fontWeight: 600 }}>u/user{item.user_id}</Text>
              ...
              {item?.user_id !== authUser?.id && (
                <TouchableOpacity
                  style={styles.btnDeleMem}
                  onPress={() => handleRemoveMember(item?.community_id, item?.user_id)}
                >
        ...
        {panelType === 'requests' &&
          infoMemberPending.map((item, index) => (
            <View
              style={[styles.cardMember, item?.user_id === authUser?.id && styles.cardMemberAd]}
              key={`idx-${index}-${item?.community_id}`}
            >
              ...
              <View style={{ marginLeft: 'auto', flexDirection: 'row', gap: 5 }}>
                <TouchableOpacity
                  style={styles.btnDeleMem}
                  onPress={() => handleDeclineMember(item?.community_id, item?.user_id)}
                >
                  <AntDesign name="close-circle" size={20} color="black" />
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.btnDeleMem}
                  onPress={() => handleApproveMember(item?.community_id, item?.user_id)}
                >
    ...
    <View style={styles.container}>
      {openOptions && (
        ...
        >
          <View style={styles.options}>
            <TouchableOpacity style={styles.btnOptions} onPress={() => handleOpenPanel('members')}>
              <Text style={{ fontWeight: 600 }}>Members</Text>
            </TouchableOpacity>

            {Number(created_by) === authUser?.id && (
              <TouchableOpacity
      ...
      <View style={styles.header}>
        <TouchableOpacity
          style={styles.back}
          onPress={() => {
            router.back(), setInfoCommunity(undefined)
          }}
        >
      ...
      <View style={styles.info}>
        <Image
          source={
            infoCommunity?.avatar && infoCommunity.avatar !== ''
              ? { uri: infoCommunity.avatar }
              : require('../../../assets/header.png')
          }
        ...
        >
          <Text
            style={{
              color: infoCommunity?.status === 'APPROVED' ? '#adadadff' : '#fff',
              fontWeight: 600,
            }}
          >
            {infoCommunity?.status === 'APPROVED' ? 'Joined' : 'Join'}
          </Text>
        </TouchableOpacity>
      </View>
    </View>
