...
import { getMyCreatedCommunities, getMyJoinedCommunities } from '../apis/communityService'
...
export default function CustomDrawer(props: any) {
  ...
  const fetchDataMyCommunity = async () => {
    ...
    try {
      const data = await getMyCreatedCommunities(authUser.id)
      setMyCommunity(data || [])
    } catch (error: any) {
  ...
  const fetchDataJoinedCommunity = async () => {
    if (!authUser?.id) return
    try {
      const data = await getMyJoinedCommunities(authUser.id)
      setJoinedCommunity(data || [])
    } catch (error: any) {
      console.log('>>> Error: ', error)
    }
  }
  ...
  return (
    <DrawerContentScrollView {...props}>
      <DrawerItem
        ...
        icon={({ color, size }) => <AntDesign name="global" size={22} color="black" />}
        onPress={() => router.push('/(protected)/(tabs)/communities')}
      />
      <DrawerItem
        label="Start a Community"
        icon={({ color, size }) => <AntDesign name="plus-circle" size={22} color="black" />}
      ...
      >
        ...
        <Text style={{ fontSize: 14, fontWeight: '600', marginLeft: 8 }}>My Communities</Text>
      </View>
      <View
        style={{
      ...
      >
        <AntDesign name="caret-right" size={18} color="black" />
        <Text style={{ fontSize: 14, fontWeight: '600', marginLeft: 8 }}>Joined Communities</Text>
      </View>
      <View
        style={{
          paddingHorizontal: 20,
